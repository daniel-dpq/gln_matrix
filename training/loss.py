...
from constants import residue_constants
...
def compute_fape(
    ...
    """
        ...
        Args:
            ...
            ca: whether to use CA atom only
        Returns:
            [*] loss tensor
    """
    # [*, N_frames, N_pts, 3]
    ...
    if not ca:
        ...
        error_dist = torch.sqrt(
            torch.squeeze((local_pred_pos[..., 1] - local_target_pos[..., 1]) ** 2, dim=-1) + eps
        )
    ...
    mask = torch.matmul(frames_mask[..., None], positions_mask[..., None, :])
    if pair_mask is not None:
        normed_error *= pair_mask   
    normalization_factor = torch.sum(mask, axis=(-1, -2))
    normed_error = (torch.sum(normed_error, axis=(-2, -1)) / (eps + normalization_factor))
...
class BackboneLoss(nn.Module):
    """Loss of Backbone"""
    def __init__(self):
        super(BackboneLoss, self).__init__()
...
class MultimerBackboneLoss(nn.Module):
    """intra_chain_bb_loss + interface_bb_loss"""
    def __init__(self):
        super(MultimerBackboneLoss, self).__init__()

    def forward(self, out, batch):
        intra_chain_bb_loss = backbone_loss(
            traj=out["frames"],
            pair_mask=batch['intra_chain_mask'],
            **{**batch},
        )
        interface_bb_loss = backbone_loss(
            traj=out["frames"],
            pair_mask=1.-batch['intra_chain_mask'],
...
class MSEBackboneLoss(nn.Module):
    """bb_loss + MSE"""
    def __init__(self):
        super(MSEBackboneLoss, self).__init__()
    ...
    def forward(self, out, batch):
        ...
        # whole mse
        mse = torch.square(
            torch.sum(batch["gt_matrix"], dim=[-1, -2], keepdim=False) - \
            torch.sum(out['gln_matrix'], dim=[-1, -2], keepdim=False)
        )
        # unit mse
        mse = torch.square(batch["gt_matrix"] - out['gln_matrix'])
...
class UnitMSEBackboneLoss(nn.Module):
    """bb_loss + MSE"""
    def __init__(self, unitmse_weight):
        super(UnitMSEBackboneLoss, self).__init__()
        self.unitmse_weight = unitmse_weight
    ...
    def forward(self, out, batch):
        ...
        if self.unitmse_weight is None:
            return mse
        ...
        return bb_loss + self.unitmse_weight*mse


class CaBackboneLoss(nn.Module):
    """bb_loss with higher Ca weight"""
    def __init__(self):
        super(CaBackboneLoss, self).__init__()
...
class InterBackboneLoss(nn.Module):
    """Only interface_bb_loss"""
    def __init__(self):
        super(InterBackboneLoss, self).__init__()
...
class LogUnitMSEBackboneLoss(nn.Module):
    """bb_loss + log(MSE)"""
    def __init__(self, unitmse_weight):
        super(LogUnitMSEBackboneLoss, self).__init__()
        self.unitmse_weight = unitmse_weight

    def forward(self, out, batch):
        true = torch.where(batch["gt_matrix"]>0, torch.log(batch["gt_matrix"]+1), -torch.log(-batch["gt_matrix"]+1))
        pred = torch.where(out["gln_matrix"]>0, torch.log(out["gln_matrix"]+1), -torch.log(-out["gln_matrix"]+1))
        # unit mse
...
class LogUnitMAEBackboneLoss(nn.Module):
    """bb_loss + log(MAE)"""
    def __init__(self, unitmae_weight):
        super(LogUnitMAEBackboneLoss, self).__init__()
        self.unitmae_weight = unitmae_weight
    ...
    def forward(self, out, batch):
        ...
        return bb_loss + self.unitmae_weight*mae


class NormedUnitMSEBackboneLoss(nn.Module):
    """bb_loss + MSE"""
    def __init__(self, unitmse_weight):
        super(NormedUnitMSEBackboneLoss, self).__init__()
        self.unitmse_weight = unitmse_weight

    def forward(self, out, batch):
        # unit mse
        mse = torch.square(1000*batch["gt_matrix"] - 1000*out['gln_matrix'])
        mse = torch.mean(mse)        
        ...
        return (1.-self.unitmse_weight)*bb_loss + self.unitmse_weight*mse


class UnitMAEBackboneLoss(nn.Module):
    """bb_loss + MAE"""
    def __init__(self, unitmae_weight):
        super(UnitMAEBackboneLoss, self).__init__()
        self.unitmae_weight = unitmae_weight

    def forward(self, out, batch):
        # unit mae
        mae = torch.abs(batch["gt_matrix"] - out['gln_matrix'])
        mae = torch.mean(mae)   
        
        if self.unitmae_weight is None:
            return mae
...
class UnitWholeMAELoss(nn.Module):
    """whl MAE + MAE"""
    def __init__(self, unitmae_weight):
        super(UnitWholeMAELoss, self).__init__()
        self.unitmae_weight = unitmae_weight
    ...
    def forward(self, out, batch):
        ...
        # whole mae
        whl_mae = torch.abs(torch.sum(batch["gt_matrix"], dim=(-1, -2)) - torch.sum(batch["gt_matrix"], dim=(-1, -2)))
        whl_mae = torch.mean(whl_mae)

        return whl_mae + self.unitmae_weight*mae


class LocalPatchedMAE(nn.Module):
    """local patched MAE"""
    def __init__(self, patch_size: int):
        super(LocalPatchedMAE, self).__init__()
        self.patch_size = patch_size

    def forward(self, out, batch):
        dim = batch["gt_matrix"].size()[-1]
        unfold = nn.Unfold(
            kernel_size=(self.patch_size, self.patch_size), 
            stride=self.patch_size,
            padding=math.ceil((self.patch_size - dim%self.patch_size)/2)
        )
        # [B, 1, N-1, N-1]
        gt_mat = batch["gt_matrix"][:, None, ...]
        # [B, Num_values_per_patch = Patch_size^2, Num_patch = (N-1)^2/Patch_size^2]
        gt_mat = unfold(gt_mat)
        # mean along each patch
        # [B, Num_patch = (N-1)^2/Patch_size^2]
        gt_mat = torch.sum(gt_mat, dim=-2)/self.patch_size/self.patch_size

        pred_mat = out['gln_matrix'][:, None, ...]
        pred_mat = unfold(pred_mat)
        pred_mat = torch.sum(pred_mat, dim=-2)/self.patch_size/self.patch_size
        # [B, N-1, N-1]
        mae_mat = torch.abs(pred_mat - gt_mat)

        mae = torch.mean(mae_mat)
...
class FocalLoss(nn.Module):
    """focal loss"""
    def __init__(self, gamma: float, clamp_mae:float):
        super(FocalLoss, self).__init__()
        self.gamma = gamma
        self.clamp_mae = clamp_mae
    ...
    def forward(self, out, batch):
        ...
        mae = torch.mean(mae)
        mae = torch.clamp(mae, min=0, max=self.clamp_mae)
        normed_error = mae / (self.clamp_mae + eps)
        focal_loss = -torch.pow(normed_error, self.gamma)*torch.log(1-normed_error)
